...
function formatSessionExpiry(expiresAt: AuthUserProfile["expiresAt"]): string {
  if (!expiresAt) {
  ...
  return new Intl.DateTimeFormat(undefined, {
    dateStyle: "medium",
    timeStyle: "short",
  }).format(new Date(String(expiresAt)));
}

function formatAuthMethod(method: AuthUserProfile["sessionMethod"]): string {
  switch (method) {
    case "bearer-session-token":
      return "Bearer token";
    case "browser-session-cookie":
  ...
  return (
    <div className="flex items-center gap-3 border-b border-border/60 px-4 py-4 sm:px-5">
      ...
      <div className="min-w-0">
        ...
        <div className="truncate text-xs text-muted-foreground">{profile.subject}</div>
      </div>
    </div>
  );
}
...
export function AccountSettingsPanel() {
  ...
  const refreshProfile = useCallback(async () => {
    ...
    try {
      ...
      setErrorMessage(error instanceof Error ? error.message : "Failed to load account profile.");
    } finally {
  ...
  const saveProfile = useCallback(async () => {
    ...
    const avatarInitials = profileAvatarInitials.trim().toUpperCase();
    if (!displayName || !avatarInitials) {
      setErrorMessage("Display name and avatar initials are required.");
      return;
    }
    ...
    try {
      ...
      setErrorMessage(error instanceof Error ? error.message : "Failed to update account profile.");
    } finally {
  ...
  const disconnectProviderAccount = useCallback(
    async (providerAccountId: ProviderAccountId) => {
      ...
      try {
        ...
        setProviderAccountErrorMessage(
          error instanceof Error ? error.message : "Failed to disconnect provider account.",
        );
      } finally {
  ...
  const connectProviderAccount = useCallback(
    async (provider: ProviderKind, accountScope: ProviderAccountConnectScope) => {
      ...
      try {
        ...
        setProviderAccountErrorMessage(
          error instanceof Error ? error.message : "Failed to prepare provider connect flow.",
        );
      } finally {
        setConnectingProvider(null);
      }
    },
    [environmentId, providerAuthThreadRef],
  );
  ...
  const confirmProviderAccount = useCallback(async () => {
    ...
    try {
      ...
      setProviderAccountErrorMessage(
        error instanceof Error ? error.message : "Failed to confirm provider account.",
      );
    } finally {
      setConfirmingProvider(null);
    }
  }, [environmentId, providerConnectSession]);

  const canSignOutSupabaseSession =
    profile?.sessionMethod === "bearer-session-token" && Boolean(readSupabaseBrowserAccessToken());

  const handleSupabaseSignOut = useCallback(() => {
    signOutSupabaseBrowserSession();
    setProfile(null);
    setErrorMessage(null);
  ...
  return (
    <SettingsPageContainer>
      <SettingsSection
        title="Account"
        headerAction={
          <Button size="xs" variant="outline" disabled={isRefreshing} onClick={refreshProfile}>
            {isRefreshing ? <LoaderIcon className="size-3.5 animate-spin" /> : null}
            Refresh
          </Button>
        }
      >
        {profile ? <AccountProfileSummary profile={profile} /> : null}
        <SettingsRow
          title="Identity"
          description="The current authenticated user mapped from the active backend session."
          status={
            errorMessage ? (
            ...
            <div className="space-y-4 border-t border-border/60 pt-3 pb-4">
              <div className="grid gap-3 sm:grid-cols-[minmax(0,1fr)_8rem_auto] sm:items-end">
                ...
                <label className="space-y-1 text-xs font-medium text-foreground">
                  <span>Avatar</span>
                  <Input
                    value={profileAvatarInitials}
                    onChange={(event) => setProfileAvatarInitials(event.target.value)}
                    maxLength={4}
                    autoComplete="off"
                  />
                </label>
                ...
                >
                  {isSavingProfile ? <LoaderIcon className="size-3.5 animate-spin" /> : null}
                  Save profile
                </Button>
              </div>
              <div className="grid gap-2 text-xs sm:grid-cols-2">
                <div>
                  ...
                  <div className="mt-1 font-medium text-foreground">{profile.role}</div>
                </div>
                <div>
                  <div className="text-muted-foreground">Session</div>
                  <div className="mt-1 font-medium text-foreground">
                    {formatAuthMethod(profile.sessionMethod)}
                  </div>
                </div>
                <div>
                  <div className="text-muted-foreground">Client</div>
                  <div className="mt-1 font-medium text-foreground">
                    {profile.client.label ?? profile.client.deviceType}
                  </div>
                </div>
                <div>
                  <div className="text-muted-foreground">Expires</div>
                  <div className="mt-1 font-medium text-foreground">
                    {formatSessionExpiry(profile.expiresAt)}
                  </div>
                </div>
              </div>
      ...
      >
        <SettingsRow
          ...
          description="Tenant-scoped provider accounts available to the current session."
          status={
            ...
            <div className="flex flex-wrap gap-2">
              {(["codex", "claudeAgent"] as const).map((provider) => (
                <Button
                  ...
                  onClick={() => void connectProviderAccount(provider, providerAccountConnectScope)}
                >
                ...
                return (
                  ...
                  >
                    <div className="min-w-0 space-y-1">
                      <div className="flex flex-wrap items-center gap-2">
                        <div className="text-sm font-semibold text-foreground">{providerLabel}</div>
                        <span
                          className={cn(
