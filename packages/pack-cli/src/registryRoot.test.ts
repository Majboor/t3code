...
describe("resolveRegistryRoot", () => {
  it("uses an explicit registry over everything else", () => {
    expect(resolveRegistryRoot("/explicit", { T3CODE_PACK_REGISTRY: "/env" }, nothing)).toBe(
      "/explicit",
    );
  });

  it("uses the configured registry when there is no explicit one", () => {
    expect(resolveRegistryRoot(undefined, { T3CODE_PACK_REGISTRY: "/env" }, nothing)).toBe("/env");
  });

  it("falls back to the default home when nothing is configured", () => {
    expect(resolveRegistryRoot(undefined, {}, nothing)).toBe(DEFAULT);
  });

  it("uses the home's own registry when that home actually has one", () => {
    expect(
      resolveRegistryRoot(undefined, { T3CODE_HOME: "/scratch" }, only("/scratch/packs")),
    ).toBe("/scratch/packs");
  });
  ...
  it("looks where the packs are when the home has no registry of its own", () => {
    ...
    // default one.
    expect(resolveRegistryRoot(undefined, { T3CODE_HOME: "/scratch" }, only(DEFAULT))).toBe(
      DEFAULT,
    );
  });

  it("keeps naming the home's registry when neither exists, so the error names the right path", () => {
    expect(resolveRegistryRoot(undefined, { T3CODE_HOME: "/scratch" }, nothing)).toBe(
      "/scratch/packs",
    );
  });

  it("ignores an empty home rather than reading it as a path", () => {
    expect(resolveRegistryRoot(undefined, { T3CODE_HOME: "" }, nothing)).toBe(DEFAULT);
  });
});
