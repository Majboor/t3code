...
try {
  ...
  check(
    "and tells you how to start",
    (await bodyText(account.page)).includes("t3 analytics declare"),
  );
  ...
  const groupByPath = account.page
    .locator('[data-testid="analytics-group"]', { hasText: "path" })
    .first();
  ...
  check(
    "it draws a bar per page, most read first",
    counted[0]?.startsWith("/about") === true && counted.length === 2,
    counted.join(" | "),
  );

  const sum = account.page
    .locator('[data-testid="analytics-aggregate"]', { hasText: "sum" })
    .first();
  await sum.click();
  await sleep(5_000);
  ...
  check(
    "summing the seconds adds both visits up",
    summed[0]?.includes("42") === true,
    summed.join(" | "),
  );
  check(
    "the refused events left nothing behind",
    !summed.some((bar) => bar.startsWith("/x")),
    summed.join(" | "),
  );
